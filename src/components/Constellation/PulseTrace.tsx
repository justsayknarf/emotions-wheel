...
export function PulseTrace({ entries, onPointClick }: Props) {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      if (t <= total) {
        const cur = posAt(t);
        const rev = Math.min(t / (PER_HOP * REVEAL_FRAC), 1);
        const ib = rev * rev * (3 - 2 * rev); // reveal tied to movement: emerges from the pulse
  ...
  return (
    <div ref={rootRef} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      {/* Launch pulse — expanding rings marking where the journey sets off.
          No filled glow, so nothing orb-like sits at the origin. */}
      {multi &&
        [0, 0.2].map((d, k) => (
          <motion.div
            key={k}
            initial={{ scale: 0.4, opacity: 0 }}
            animate={{ scale: [0.4, 4.8], opacity: [0, 0.5, 0] }}
            transition={{ delay: d, duration: 1.1, times: [0, 0.18, 1], ease: 'easeOut' }}
            style={{
              position: 'absolute',
              left: `${points[0].lx}%`,
              top: `${points[0].ly}%`,
      ...
      {points.map((p, i) => (
        <div key={p.entry.id}>
          ...
          <motion.button
            ...
            transition={{ delay: i === 0 ? PER_HOP * REVEAL_FRAC : arrival(i), duration: 0.35, ease: 'easeOut' }}
